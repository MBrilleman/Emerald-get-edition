...
#include "event_data.h" //shop_items_per_badge_count
...
{
    u16 i = 0;
    u8 badgeCount = GetNumberOfBadges();//shop_items_per_badge_count
    if (items == NULL) //shop_items_per_badge_count 
        sMartInfo.itemList = sShopInventories[badgeCount];//shop_items_per_badge_count
    else //shop_items_per_badge_count
        sMartInfo.itemList = items;//shop_items_per_badge_count
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId)
{
    ...
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        if ((tItemId == ITEM_POKE_BALL) && tItemCount > 9 && AddBagItem(ITEM_PREMIER_BALL, tItemCount / 10) == TRUE)
           if (tItemCount > 19)
           {
